...
import (
	...
	"github.com/setcreed/hade-kit/tools/randoms"
...
func (s *service) verifyCode(req VerifyCodeReq) (*VerifyCodeResp, error) {
	...
	if err != nil {
		logs.Errorf("verifyCode generateResetPasswordToken error: %v", err)
		return nil, biz.ErrTokenGen
	}
	// 删除redis中的验证码 设置1秒过期
	defer func(cache *cache.RedisCache, key string, value string, expire int64) {
		err := cache.Set(key, value, expire)
		if err != nil {
			logs.Errorf("verifyCode Set error: %v", err)
			return
		}
	}(s.cache, codeKey, "", 1)
...
func (s *service) resetPassword(c context.Context, resetReq ResetPasswordReq) (any, error) {
	...
	defer func(cache *cache.RedisCache, key string, value string, expire int64) {
		err := cache.Set(key, value, expire)
		if err != nil {
			logs.Errorf("resetPassword Set error: %v", err)
			return
		}
	}(s.cache, tokenKey, "", 1)
